...
7. **Локализация по карте** ([LOCALIZATION.md](LOCALIZATION.md)): `robot.launch.py perception:=true localization:=true`. Первый запуск строит карту: встать, осмотр (клавиша `4`), медленно обойти комнату. Карта сохраняется в `~/.ros/dog_map.*` при остановке. Во втором запуске встать в другом месте, сделать осмотр: в журнале `relocalization ... found`, `localization/status` — `tracking`. Сверить: поставить робота на метку на полу, пройти круг, вернуть на метку — `localization/pose` должна показать то же место в пределах 5 см.
