...
3. **При центральном импульсе** надеть качалку так, чтобы сустав стоял в опорной позе:
   - hip — нога вертикально;
   - thigh — угол из `offset_deg` по умолчанию в `servos.yaml`;
   - calf — прямой угол назад.

   Шлицы дают шаг ~7–10°, остаток уберёт калибровка.
4. **Качнуть каждую серву** импульсами 1370 ± 200 мкс (≈ ±20°). Звено не должно упираться. Если упирается — сузить `min_deg`/`max_deg` в `servos.yaml` до калибровки.
5. `pca9685_probe off` после каждой сервы.

**Критерий перехода:**
- все 12 серв отвечают на своём канале;
- качалки надеты в опорной позе;
...
**Критерий перехода:**
- в опорной позе все четыре ноги одинаковы с точностью 1–2°;
...
3. **На каждом шаге:** 3 прохода. Если хоть один раз падение или сход вбок больше 15 см — остаёмся на предыдущем уровне.
4. Записать реальные пределы в таблицу [TERRAIN.md](TERRAIN.md) рядом с симуляцией.
...
1. **VL53L1X FL/FR:**
   ...
   - снять поправки на ровном полу, записать в `perception.tof_offsets`.
2. **Лидары крестом:**
...
3. **`perception_node`:**
   - сначала на ноутбуке по Wi-Fi, потом на плате;
   - для платы переписать на C++ или собрать ToF в одно сообщение (см. [PERCEPTION.md](PERCEPTION.md), раздел «Вычислительная нагрузка»).
4. **Сверка с симуляцией:** «стоп перед ступенькой» в `locomotion` — сначала в Gazebo тестом, потом на железе с доской 30 мм.
