...
Команда `survey` (кнопка «Осмотр», клавиша `4`) принимается только из стойки, стоя на месте. Стопы остаются на месте, корпус плавно (min-jerk) наклоняется носом вверх на 12° и вниз на 10°, два раза, потом поворачивается по рысканию на ±15°, тоже два раза, и возвращается в стойку. Всего ~12 с, состояние `survey`. Код — `dog_control/survey.hpp`, параметры — `robot.yaml`, раздел `survey`.

Зачем: лидары наклонены на 30° крестом, и неподвижный корпус видит только две косые линии вокруг себя. Качание проводит их по стенам и мебели выше, ниже и по сторонам. Так заполняется карта для локализации, а на старте робот находит себя в сохранённой карте, прежде чем идти.

Тест (`test_locomotion.cpp`, `SurveyLooksAroundWithTheFeetWhereTheyStand`) проверяет: команда отклоняется на ходу, все цели в досягаемости, стопы в мире стоят на месте с точностью 1 мм, крайние углы достигаются, в конце — та же стойка.

**В симуляции** тангаж отрабатывается как задан, а рыскание — около ±9° из ±15°, и робот за осмотр уползает на ~0,13 м. Команды суставов согласованы (расстояния между стопами меняются не больше чем на 2 мм — это перекатывание сферической стопы), но четыре опоры с жёсткими суставами в скоростном режиме и мягкий контакт DART проскальзывают. Приветствие сдвигается так же. Поэтому проверка в CI (`perception_check --survey`) требует лишь размаха рыскания ≥ 12° и сдвига ≤ 0,25 м. На роботе проверить на полу, со стопами в резине.
...
| осмотр | проходит (в CI) | тангаж −11°…+9°, рыскание ±9° из ±15°, сдвиг ~0,13 м |
