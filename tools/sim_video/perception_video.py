...
def main():
    ...
    scans = {n: [s for s in rec['scans'] if s['name'] == n] for n in ('lidar_left', 'lidar_right')}
        ...
        for name, edge, k, corr, tofs in feats:
            for fam in ('lidar', 'tof'):
                ...
                for h in hazards:
                    if h['kind'] != k:
                        continue
                    ok = (h['source'] == 'lidar' and h.get('corridor') in corr) if fam == 'lidar' \
    ...
    for n_out, t in enumerate(t_out):
        ...
        a_hz.set_yticks(range(4))
        a_hz.set_yticklabels([x[0] for x in rows], fontsize=7)
        a_hz.set_ylim(3.6, -0.6)
        ...
        if feat_order:
            for k, name in enumerate(feat_order):
                y = 0.905 - k * 0.05
                parts = []
                for fam, lab in (('lidar', 'лидары'), ('tof', 'ToF')):
                    b = firsts.get((name, fam))
                    parts.append(f'{lab} {b[1]:.2f} м' if b and b[0] <= t else f'{lab} —')
                seen = any(firsts.get((name, fam)) and firsts[(name, fam)][0] <= t for fam in ('lidar', 'tof'))
